...
// Set chip model.
// ----------------------------------------------------------------------------
void C64_SID::set_chip_model(chip_model model)
{
  for (int i = 0; i < 3; i++) {
...
// Write registers.
// ----------------------------------------------------------------------------
void C64_SID::write(reg8 offset, reg8 value)
{
  bus_value = value;
...
// Read state.
// ----------------------------------------------------------------------------
C64_SID::State C64_SID::read_state()
{
  State state;
  int i, j;
...
// Write state.
// ----------------------------------------------------------------------------
void C64_SID::write_state(const State& state)
{
  int i;
...
// Enable filter.
// ----------------------------------------------------------------------------
void C64_SID::enable_filter(bool enable)
{
...
// ----------------------------------------------------------------------------
void C64_SID::enable_external_filter(bool enable)
{
...
// not overfilled.
// ----------------------------------------------------------------------------
bool C64_SID::set_sampling_parameters(double clock_freq, sampling_method method,
				  ...
				  double filter_scale)
{
  ...
  int n = (int)ceil(log(res/f_cycles_per_sample)/log((double)((double)2)));
  fir_RES = 1 << n;
...
// ----------------------------------------------------------------------------
void C64_SID::adjust_sampling_frequency(double sample_freq)
{
  cycles_per_sample =
...
// ----------------------------------------------------------------------------
void C64_SID::fc_default(const fc_point*& points, int& count)
{
...
// ----------------------------------------------------------------------------
PointPlotter<sound_sample> C64_SID::fc_plotter()
{
...
// ----------------------------------------------------------------------------
void C64_SID::clock(cycle_count delta_t)
{
  int i;
...
// 
// ----------------------------------------------------------------------------
int C64_SID::clock(cycle_count& delta_t, short* buf, int n, int interleave)
{
  switch (sampling) {
  default:
  case SAMPLE_FAST:
...
// ----------------------------------------------------------------------------
RESID_INLINE
int C64_SID::clock_fast(cycle_count& delta_t, short* buf, int n,
		    int interleave)
{
  int s = 0;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int C64_SID::clock_interpolate(cycle_count& delta_t, short* buf, int n,
			   int interleave)
{
  int s = 0;
  int i;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int C64_SID::clock_resample_interpolate(cycle_count& delta_t, short* buf, int n,
				    int interleave)
{
  int s = 0;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int C64_SID::clock_resample_fast(cycle_count& delta_t, short* buf, int n,
			     int interleave)
{
  int s = 0;
